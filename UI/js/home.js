$(document).ready(function () {
    $('.navbar-light').hover(function () {
        ...
        $('.navbar-light').css('background-color', '#7c1629')
    })
...
function purchaseItem(money, item) {
    ...
    $.ajax({
        type: 'GET',
        url: 'http://localhost:8081/api/items/' + item + '/' + money,
        success: function (data) {
            ...
            $('#change-money').text(data.dollar + ' Dollar '+ data.quarter + ' Quarter ' + data.dime + ' Dimes ' + data.nickel + ' Nickels '
                + data.leftOverPennies + ' Pennies');
            //Refresh Item list
            loadItems();
        },
        ...
        success: function (data) {
            ...
            $.each(data, function (index, allItems) {
                ...
                itemInfo = '<div class="col-sm-4">',
                    ...
                    itemInfo += '<p id="item-id">' + allItems.itemid + '</p>' + '<br>',
                    itemInfo += '<p id="item-name">' + allItems.itemName + '</p>' + '<br>' + '</h5>',
                    itemInfo += '<p id="item-price"> $' + allItems.itemCost + '</p>',
                    itemInfo += '<h5 id="item-qty" class="bg-info border border-success rounded-pill">' + 'Quantity Left:' + allItems.numberOfItemsInInventory + '</h5>',
